...
CREATE POLICY "Users can view own responses" ON public.assessment_responses
  FOR SELECT TO authenticated USING (
    EXISTS (SELECT 1 FROM public.assessments a WHERE a.id = assessment_id AND (a.user_id = auth.uid() OR public.has_role(auth.uid(), 'admin')))
  );
...
CREATE POLICY "Users can update own responses" ON public.assessment_responses
