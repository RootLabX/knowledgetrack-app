...
CREATE TABLE mapper.achievements (
...
CREATE TABLE mapper.user_achievements (
  id uuid NOT NULL DEFAULT gen_random_uuid() PRIMARY KEY,
  user_id uuid NOT NULL,
  achievement_id uuid NOT NULL REFERENCES mapper.achievements(id) ON DELETE CASCADE,
  earned_at timestamp with time zone NOT NULL DEFAULT now(),
  UNIQUE(user_id, achievement_id)
);

-- Create user_objectives table for personal goals
CREATE TABLE mapper.user_objectives (
  id uuid NOT NULL DEFAULT gen_random_uuid() PRIMARY KEY,
  user_id uuid NOT NULL,
  title text NOT NULL,
  description text,
...
ALTER TABLE mapper.user_objectives ENABLE ROW LEVEL SECURITY;

-- Achievements policies (everyone can view)
CREATE POLICY "Everyone can view achievements" ON mapper.achievements FOR SELECT USING (true);
CREATE POLICY "Admins can manage achievements" ON mapper.achievements FOR ALL USING (public.has_role(auth.uid(), 'admin'::app_role));

-- User achievements policies
CREATE POLICY "Users can view own achievements" ON mapper.user_achievements FOR SELECT USING (auth.uid() = user_id OR public.has_role(auth.uid(), 'admin'::app_role));
CREATE POLICY "System can insert achievements" ON mapper.user_achievements FOR INSERT WITH CHECK (auth.uid() = user_id);

-- User objectives policies
CREATE POLICY "Users can view own objectives" ON mapper.user_objectives FOR SELECT USING (auth.uid() = user_id OR public.has_role(auth.uid(), 'admin'::app_role));
CREATE POLICY "Users can manage own objectives" ON mapper.user_objectives FOR INSERT WITH CHECK (auth.uid() = user_id);
CREATE POLICY "Users can update own objectives" ON mapper.user_objectives FOR UPDATE USING (auth.uid() = user_id);
CREATE POLICY "Users can delete own objectives" ON mapper.user_objectives FOR DELETE USING (auth.uid() = user_id);

-- Insert default achievements
INSERT INTO mapper.achievements (name, description, icon, category, points, criteria_type, criteria_value) VALUES
